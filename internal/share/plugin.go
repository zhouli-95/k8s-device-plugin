...
var _ pluginapi.DevicePluginServer = &SharePlugin{}
var _ plugin.Plugin = &SharePlugin{}
...
func NewDevicePlugin(ctx context.Context, ch <-chan struct{}, opts *option.Options) *SharePlugin {
	p := SharePlugin{
		name:       fmt.Sprintf("shared-%s", resource.GetResourceName()),
		ctx:        ctx,
		devList:    []*resource.ComputeDevice{},
...
func (p *SharePlugin) Start() error {
	...
	klog.Infof("Plugin device list %v", p.devList)
...
func (p *SharePlugin) Stop() error {
	klog.Infof("Plugin stop")
	return nil
}

func (p *SharePlugin) createCDISpec() error {
	handler := func(spec *specs.Spec) error {
		for _, dev := range p.devList {
			for i := 0; i < p.options.ShareOption.ShareNumber; i++ {
...
func (p *SharePlugin) GetDevicePluginOptions(context.Context, *pluginapi.Empty) (*pluginapi.DevicePluginOptions, error) {
...
func (p *SharePlugin) ListAndWatch(e *pluginapi.Empty, s pluginapi.DevicePlugin_ListAndWatchServer) error {
...
func (p *SharePlugin) genDevList() []*pluginapi.Device {
...
func (p *SharePlugin) Allocate(ctx context.Context, reqs *pluginapi.AllocateRequest) (*pluginapi.AllocateResponse, error) {
...
func (p *SharePlugin) allocateByNative(req *pluginapi.ContainerAllocateRequest) (*pluginapi.ContainerAllocateResponse, error) {
	...
	for _, id := range req.DevicesIDs {
		devId, _ := parseShareUUID(id)
		dev := p.devList[p.devMap[devId]]
		resp.Devices = append(resp.Devices, &pluginapi.DeviceSpec{
...
func (p *SharePlugin) allocateByCDIAnnotation(req *pluginapi.ContainerAllocateRequest) (*pluginapi.ContainerAllocateResponse, error) {
	resp := pluginapi.ContainerAllocateResponse{}
...
func (p *SharePlugin) allocateByCDICRI(req *pluginapi.ContainerAllocateRequest) (*pluginapi.ContainerAllocateResponse, error) {
	resp := pluginapi.ContainerAllocateResponse{}

	var devs []string
	for _, id := range req.DevicesIDs {
		devs = append(devs, p.cdiHandler.QualifiedName(p.name, id))
	}
...
func (p *SharePlugin) PreStartContainer(ctx context.Context, req *pluginapi.PreStartContainerRequest) (*pluginapi.PreStartContainerResponse, error) {
	klog.Infof("Reset device list %v", req.DevicesIDs)
...
func (p *SharePlugin) GetPreferredAllocation(ctx context.Context, req *pluginapi.PreferredAllocationRequest) (*pluginapi.PreferredAllocationResponse, error) {
	return &pluginapi.PreferredAllocationResponse{}, nil
}

func genShareUUID(uuid string, i int) string {
	return fmt.Sprintf("%s::%02d", uuid, i)
}

func parseShareUUID(uuid string) (string, int) {
	arr := strings.Split(uuid, "::")
	num, _ := strconv.ParseInt(arr[1], 10, 32)
	return arr[0], int(num)
}
